// src/index.js — Cloudflare Worker для семейного дерева (шаблон)
...
export default {
  async fetch(request, env) {
    ...
    if(path === '/api/upload-photo' && method === 'POST') {
      ...
      // Person existence verified against IDX.nodes in KV before writing
      // to R2, so an upload never creates an orphaned file for a
      // nonexistent/typo'd person id.
      ...
      if(!IDX.nodes[person]) return err('Персона не найдена: ' + person, 404);
      ...
      // Random suffix (beyond person-timestamp) makes the R2 key
      // unpredictable — GET /api/photo/:key below is intentionally
      // unauthenticated, so key guessability is its only access control.
      ...
      // Built from the incoming request's own origin — not a hardcoded
      // domain — since every template deployment lives on its own
      // Worker subdomain.
      const origin = new URL(request.url).origin;
      return json({ ok: true, key, url: origin + '/api/photo/' + key });
    }

    // ── GET /api/photo/:key ───────────────────────────────
    // Public: serves an uploaded photo from R2 by its key. No auth — the
    // value is embedded directly as <img src="..."> in the person panel,
    // which can't attach X-Password. The panel itself still gates
    // *display* of the <img> behind login.
    ...
    if(path === '/calendar.ics' && method === 'GET') {
      ...
      let ics = [
        'BEGIN:VCALENDAR',
        'VERSION:2.0',
        `PRODID:-//${env.FAMILY_NAME_RU || 'Family Tree'}//Birthday Calendar//RU`,
        'CALSCALE:GREGORIAN',
        'METHOD:PUBLISH',
        `X-WR-CALNAME:🎂 Дни рождения — ${env.FAMILY_NAME_RU || 'Family Tree'}`,
        `X-WR-CALDESC:Дни рождения семьи ${env.FAMILY_NAME_RU || 'Family Tree'}`,
      ...
      for(const [id, n] of Object.entries(nodes)) {
        ...
        const uid     = `birthday-${id}@${env.CALENDAR_UID_DOMAIN || 'family-tree'}`;
        const name    = n.name;
      ...
      return new Response(ics.join('\r\n'), {
        headers: {
          ...
          'Content-Disposition': `inline; filename="${env.FAMILY_SLUG || 'family'}-birthdays.ics"`,
    ...
    if(path === '/contacts.vcf' && method === 'GET') {
      ...
      for(const [id, n] of Object.entries(nodes)){
        ...
        lines.push('ORG:' + (env.FAMILY_NAME_RU || 'Family'));
      ...
      return new Response(lines.join('\r\n'), {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="${env.FAMILY_SLUG || 'family'}-contacts.vcf"`,
          'Cache-Control': 'max-age=3600',
        }
      });
    }
    ...
    // Mirrors the same endpoint already deployed in the mostovoy-tree
    // production Worker.
...
// requiring a redeploy — see PATCH /api/settings/guest-password.
// Falls back to the deploy-time hash in wrangler_template.toml if no override exists.
